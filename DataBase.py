...
# C:\Users\eloyfernandez\Documents\Uni\integracion de datos\DataIntegrationScrapper\
	...
	conn.commit()
	list_of_dfs[1].to_sql('museums', conn, if_exists='append', index = False)
	...
	conn.commit()
	list_of_dfs[2].to_sql('tvguia', conn, if_exists='append', index = False)
	...
	conn.commit()
	list_of_dfs[4].to_sql('weather', conn, if_exists='append', index = False)
	...
	conn.commit()
	list_of_dfs[5].to_sql('cinemas', conn, if_exists='append', index = False)
	...
	conn.commit()
	list_of_dfs[6].to_sql('cultural_places', conn, if_exists='append', index = False)
